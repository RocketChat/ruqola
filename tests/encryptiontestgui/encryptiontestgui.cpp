...
EncryptionTestGui::EncryptionTestGui(QWidget *parent)
    : QWidget{parent}, mTextEdit(new QTextEdit(this)),
...

{
  ...
  auto pushButtonDeriveMasterKey =
      new QPushButton(QStringLiteral("Derive Master Key"), this);
  ...
  connect(pushButtonDeriveMasterKey, &QPushButton::clicked, this, [this]() {
    ...
    mMasterKey = EncryptionUtils::deriveKey(userId.toUtf8(), password.toUtf8(),
                                            1000, 32);
    qDebug() << "Derived Master Key:" << mMasterKey.toHex();
    mTextEditResult->setPlainText(
        QStringLiteral("Master Key derived successfully!"));
  });

  auto pushButtonGenerateRSAKey =
      new QPushButton(QStringLiteral("Generate RSA Pair"), this);
  mainLayout->addWidget(pushButtonGenerateRSAKey);
  connect(pushButtonGenerateRSAKey, &QPushButton::clicked, this, []() {
    // test
  });
  auto pushButtonGenerateSessionKey =
      new QPushButton(QStringLiteral("Generate Session Key"), this);
